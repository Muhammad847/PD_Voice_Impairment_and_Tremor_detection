...
from ema import EMA
from datasets_naqvi import DrawDataset
from transforms import RandomRotation
from models.modelM3 import ModelM3
from models.modelM5_nqv import ModelM5
from models.modelM7 import ModelM7

def run(p_seed=0, p_epochs=50, p_kernel_size=5, p_logdir="temp"):
    ...
    # data loader -----------------------------------------------------------------#
    train_dataset = DrawDataset(training=True, transform=transform)
    test_dataset = DrawDataset(training=False, transform=None)
    ...
    if KERNEL_SIZE == 3:
        ...
        raise ValueError("Invalid KERNEL_SIZE value: %d" % KERNEL_SIZE)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            t_v = target.view_as(train_pred)
            t_eq = train_pred.eq(t_v)
            train_corr += t_eq.sum().item()
            ...
            ema(model, g_step)
            if batch_idx % 100 == 0:
                # print('Train Epoch: {} [{:02d}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                #     epoch, batch_idx * len(data), len(train_loader.dataset),
                #     36. * batch_idx / len(train_loader), loss.item()))
...
if __name__ == "__main__":
    ...
    p.add_argument("--seed", default=3, type=int)
